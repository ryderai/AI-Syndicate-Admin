...
/* WHY A REQUEST DIDN'T WORK, IN PLAIN ENGLISH — 25 Sep 2026.
 * Andrew: "Let's see if we can go in even more detail." Ryder: deeper, still
 * understandable, "so we can hone in on usage and see exactly what the issues
 * are." The rollup (migration 0043) hands back the AI company's answer code;
 * this turns it into six groups a person can act on. */
export const OUTCOMES = {
  ok: { label: "Worked", color: "#16a34a", what: "The AI answered." },
  busy: { label: "Turned away (too busy)", color: "#f59e0b", what: "The AI company said we sent too many requests at once, or hit a usage limit (code 429). Usually nothing is charged. It fixes itself, or needs slower sending or a higher limit." },
  refused: { label: "Refused (key or access)", color: "#dc2626", what: "The AI company would not accept our key (401) or our account is not allowed to use it (403). Every request fails until the key or account is fixed." },
  rejected: { label: "Rejected (bad request)", color: "#b45309", what: "The AI company said the request itself was wrong (400 and similar). The exact message is not saved yet. (Anthropic also answers this way when a spending limit is hit.)" },
  down: { label: "AI company error", color: "#7c3aed", what: "The AI company had a problem on its end (500-type codes, or \"overloaded\")." },
  timeout: { label: "Timed out", color: "#64748b", what: "We stopped waiting for an answer. The AI company may still have done (and charged for) the work." },
  unknown: { label: "Failed (reason not saved)", color: "#94a3b8", what: "The request failed but the reason was not recorded." },
};
export const OUTCOME_ORDER = ["ok", "busy", "refused", "rejected", "down", "timeout", "unknown"];

/** Which of the groups above a rollup reason belongs to. */
export function outcomeOf(reason) {
  const r = String(reason ?? "unknown").toLowerCase();
  if (r === "ok") return "ok";
  if (r === "429" || r === "rate_limited") return "busy";
  if (r === "401" || r === "403") return "refused";
  if (r === "timeout" || r === "408" || r === "network_error") return "timeout";
  if (/^5\d\d$/.test(r) || r === "529") return "down";
  if (/^4\d\d$/.test(r) || r === "http_error" || r === "rejected" || r === "unparseable" || r === "cut_off") return "rejected";
  return "unknown";
}

const CODE_WORDS = {
  429: "too many requests", 401: "key not accepted", 403: "not allowed", 400: "bad request",
  404: "not found", 413: "request too big", 422: "request not understood", 500: "server error",
  502: "bad gateway", 503: "unavailable", 504: "gateway timeout", 529: "overloaded",
};

/** "Turned away (too busy) · code 429, too many requests" */
export function reasonLabel(reason) {
  const o = OUTCOMES[outcomeOf(reason)];
  const r = String(reason ?? "");
  if (/^\d{3}$/.test(r)) return `${o.label} · code ${r}${CODE_WORDS[r] ? `, ${CODE_WORDS[r]}` : ""}`;
  if (r === "http_error") return `${o.label} · error code not saved`;
  return o.label;
}
