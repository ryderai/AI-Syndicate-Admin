 ...
 */
/* `idColumn` — the unique tie-breaker column. Almost every table is `id`, but
 * hs_lead_sources is keyed on `lead_id` and has no `id` at all; ordering on a
 * column that does not exist makes PostgREST refuse the whole query, and the
 * page then reads "nothing here" over a table full of rows. Found 16 Sep 2026
 * on the Home Services page: three captured leads, "No leads yet". */
export async function fetchPaged(build, { order = "created_at", ascending = false, max = 50000, idColumn = "id" } = {}) {
  const rows = [];
