...
/* The key for a call that never said what it was doing. Named, not blank, so
 * it sorts and reads as the gap it is. The leading marker keeps it out of the
 * namespace a real job could occupy: platform job names are dotted plan-token
 * keys, so none of them can start with "— ". Without that, a job somebody
 * named "not labelled yet" would merge into the gap bucket silently. */
export const UNLABELLED = "— not labelled yet —";

/* Console calls are prefixed so a console `client_report` can never be added
 * to a platform job of the same name. Two systems, two meanings, one page. */
export const CONSOLE_PREFIX = "console · ";

export const STATUSES = ["ok", "failed", "rejected", "capped", "legacy"];

/* WHAT EACH STATUS MEANS, in the words the page is allowed to use.
 *
 * These are four DIFFERENT things and the page used to print three of them as
 * one word. On 7 Sep 2026, 384 Mistral calls that the provider had turned away
 * with an HTTP 429 sat on this page under "calls failed before the AI reported
 * anything" — so a rate limit read as a broken integration, and nobody could
 * tell the two apart from any screen.
 *
 *  ok        the call went through and the provider answered.
 *  capped    the call STOPPED AT A CAP. Two different caps reach this status
 *            and they bill differently, so nothing here may claim either way:
 *              - the PROVIDER's cap: an HTTP 429 rate limit
 *                (platform lib/ai-meter.js). Nothing generated, nothing billed.
 *              - OUR OWN cap: the agent ran out of rounds
 *                (lib/ai-agent.js sets cappedOut at AGENT_MAX_ROUNDS; read by
 *                api/ai-chat.js, api/console-report.js, api/rep-report.js).
 *                Every round it DID run was a billed request, so these rows
 *                carry real tokens and a real cost_micros — they are among the
 *                most expensive calls in the system.
 *            An early version of this file said capped meant "rate limited,
 *            nothing billed". That was true of the 384 Mistral rows that
 *            prompted the change and false of every cappedOut row, on a screen
 *            whose whole job is money. The Cost column on the row is the only
 *            thing that knows; the label must not guess.
 *  rejected  the AI DID answer and we threw the answer away, because it did
 *            not pass our own checks. The provider generated tokens, so this
 *            one costs money even though nothing was delivered.
 *  failed    the call errored: a timeout, a network drop, or a provider error
 *            that was not a rate limit. Whether it was billed depends on how
 *            far it got, and this row cannot tell you.
 *  legacy    a pre-28-Aug-2026 row costed with one hardcoded price. History,
 *            not a measurement. Counted as ok, and separately as legacy.
 *
 * A status outside this list is a bug, not a new category, so readers fall
 * back to "ok" rather than inventing one. */
...
export function cacheSavingMicros(price, usage) {
  if (!price) return null;
  /* NO USAGE OBJECT AT ALL MEANS WE DO NOT KNOW — the same rule costMicros()
   * has always followed, and this function did not. It ran normalizeUsage(null)
   * into an empty object, found no cached tokens, and returned 0: "the cache
   * saved us nothing", stated as a measurement, about a call that never told us
   * how many tokens it used. priceCall() therefore returned an UNPRICED cost
   * next to a confidently measured saving of zero.
   * BE PRECISE ABOUT THE BLAST RADIUS, because an earlier draft of this
   * comment overstated it. Nothing on the AI Cost page or in the database ever
   * differed: the single caller (lib/ai-usage.js) stores it as
   * `priced.savingMicros ? { cacheSavingMicros: … } : {}`, and 0 and null are
   * both falsy, so the key was omitted either way. What was wrong was this
   * function's ANSWER, and the next caller to treat 0 as a measurement would
   * have inherited it.
   * A call that DID report usage and read nothing from a cache still returns 0
   * below, because that one really did save nothing. Found 7 Sep 2026 by a
   * checker reading this file against costMicros(); tests/ai-prices covers it. */
  if (usage === null || usage === undefined) return null;
  const u = normalizeUsage(usage);
...
function addEvent(t, ev) {
  ...
  /* FOUR BUCKETS, NOT TWO. `failed` used to be "anything that is not ok", so a
   * rate limit and a thrown-away answer both came out as failures — and the
   * page printed the total under a sentence saying the call broke. `failed`
   * now means ONLY status === "failed". The superset lives on `notOk` for the
   * one or two places that genuinely want "did not deliver", and every bucket
   * is named on screen. STATUSES is closed and an unknown status has already
   * fallen back to "ok" above, so the final `else` is exactly "failed". */
  if (status === "ok" || status === "legacy") t.ok += 1;
  else if (status === "capped") t.capped += 1;
...
function finishTotals(t) {
  ...
  return {
    ...
    capped: t.capped,
    rejected: t.rejected,
    /* Everything that did not deliver an answer we kept, whatever the reason.
     *
     * NOTHING ON A SCREEN READS THIS TODAY — the only readers are
     * tests/ai-cost/test.mjs, and an earlier comment here claimed "one or two
     * places" that did not exist. It is kept for one reason: `failed` narrowed
     * from "not ok" to "status === failed" in this change, and the next person
     * who wants the old superset must be able to ask for it BY NAME instead of
     * reading `failed` and being quietly wrong by 384 calls. calls = ok + notOk
     * always, and the tests assert it. Delete it only together with that
     * assertion. */
    notOk: t.failed + t.capped + t.rejected,
...
export const GROUPINGS = {
  ...
  /* ⭐ BY JOB — what the call was actually DOING.
   *
   * `platform_feature` has been written by api/usage-ingest.js since 0032 and
   * grouped on by nothing, so the one column that answers "what is this spend
   * for" was collected and never shown. Meanwhile "By feature" reads the
   * CONSOLE's own enum, which every platform row leaves at "other" — so on
   * 8 Sep 2026 that view was a single row: `other · 1,655 calls · $11.08`.
   * Six ways to cut the spend and four of them showed one line.
   *
   * The two columns are not the same thing and are not merged into one key:
   *   platform_feature   the platform's job name (`brand.scan`, `audit.run`)
   *   feature            the console's own enum (`client_report`, `assistant`)
   * A console row is prefixed so it can never be mistaken for a platform job
   * of the same name, and a row carrying neither says so in words rather than
   * hiding in a bucket called "other" — an unlabelled call is a gap in the
   * books, not a category of work. */
  job: {
    label: "By job",
    keyOf: (ev) => {
      /* TRIMMED, and whitespace counts as absent. api/usage-ingest.js only
       * slices this field to 120 chars — it does not trim or reject — so a
       * label of "   " is truthy, gets its own group, and renders as a
       * visually BLANK table row with a dollar figure beside it. A blank row
       * on a money page reads as a rendering bug, and hides the fact that the
       * spend was never labelled. */
      const pf = typeof ev?.platform_feature === "string" ? ev.platform_feature.trim() : "";
      if (pf) return pf;
      // The console labels its own calls; the platform mostly does not yet.
      const f = typeof ev?.feature === "string" ? ev.feature.trim() : "";
      if (ev?.source === "admin" && f) return `${CONSOLE_PREFIX}${f}`;
      return UNLABELLED;
    },
    labelOf: (key) => key,
  },
