...
const SCOPE_BY_ROLE = {
  owner: ["clients", "tasks", "weekly", "leads", "leadActivity", "leadSources", "tickets",
    ...
    "companies", "leadLists", "proposals", "clientReports", "platformAccounts"],
  ...
  if (canSee(role, "clientReports") && snap.clientReports?.length) {
    out.push(section("WHAT WE HAVE ALREADY WRITTEN ABOUT EACH CLIENT (newest first — the headline only)",
      snap.clientReports.map((r) => {
        /* First line of the summary only. A full report runs to 1,200 words and
         * twenty of them would BE the prompt. */
        const first = short(String(r.summary || "").split("\n").find((l) => l.trim()) || "", 180);
