...
const SCOPE_BY_ROLE = {
  ...
  /* No "money" for an admin since 24 Sep 2026 — money is owners only (Ryder,
   * Andrew, CJ). The service role ignores row-level security, so this list is
   * the guard for anything the assistant or a report reads. */
 ...
 * @param userId  the person asking. Used for "yours" markers — AND, for a sales
 *                rep, to filter the leads and the lead activity, since 30 Aug
 *                2026. See repLeadFilter below.
...
export async function loadSystemContext(admin, { role = "sales", userId = null, focus = {} } = {}) {
  ...
  /* ---- A REP'S AI SEES WHAT A REP'S SCREEN SEES — 30 Aug 2026 ----
   *
   * Ryder reversed the visibility rule that day: a sales rep no longer sees a
   * lead somebody else holds. `visibleToMember` in src/lib/salesSheet.js is that
   * rule for the Sales page. THIS is the same rule for the AI, and it has to be
   * here because the two are separate doors into the same rows.
   *
   * An adversarial review found the door standing open: a rep could open AI
   * Brain, ask "which leads are going cold", and the context block handed the
   * model every other rep's open leads WITH THE HOLDER'S NAME on each line —
   * the exact thing that had just been taken off their screen. The rep-brief
   * panel meanwhile said, in as many words, "Reads your leads, your firms and
   * your own follow-ups. Nothing else."
   *
   * This client runs on the SERVICE ROLE, which ignores row-level security, so
   * this line is the guard — the same shape, and for the same reason, as the
   * reminders filter thirty lines below, which an Aug 20 review found missing.
   *
   * The rule, spelled out because it must not drift from salesSheet.js:
   *   a rep may see a lead they own, or a lead nobody owns. Nothing else. */
  const repScoped = role === "sales" && userId;
  const repLeadFilter = (q) => (repScoped ? q.or(`owner_id.eq.${userId},owner_id.is.null`) : q);
  ...
  if (has("leads")) {
    jobs.push(Promise.all([
      /* BOTH reads are scoped. Filtering one of the two would have left the
         other one carrying exactly the rows the first refused. */
      readTable(admin, "admin_leads", (q) => repLeadFilter(q)
  ...
  /* Money is owners only (since 24 Sep 2026), and it is the whole invoice row: the report
  ...
  /* ---- AND THE ACTIVITY THAT BELONGS TO LEADS A REP MAY NOT SEE ----
   *
   * admin_lead_activity is read by lead-less date order, so scoping it in the
   * query would mean either a second round trip for the id list or a filter on a
   * column it does not have. It is pruned here instead, against the leads that
   * actually came back — which is the only set that can be right, because it is
   * the same set the context block will print.
   *
   * Hiding a lead and then printing its call notes underneath would be the leak
   * with an extra step. Done for the rep role only; nothing changes for an owner
   * or an admin. */
  if (repScoped && Array.isArray(snap.leadActivity)) {
    const seen = new Set((snap.leads || []).map((l) => l.id));
    const before = snap.leadActivity.length;
    snap.leadActivity = snap.leadActivity.filter((a) => !a.lead_id || seen.has(a.lead_id));
    /* Said out loud rather than silently, so a thin activity list reads as
       "scoped" and not as "nothing has happened". */
    snap.leadActivityScoped = before !== snap.leadActivity.length;
  }
