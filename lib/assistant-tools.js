...
const LEAD_STAGES = ["new", "contacted", "follow_up", "meeting", "proposal", "won", "lost"];
  ...
  try {
    switch (name) {
      ...
      case "search": {
        ...
        const { data, error } = await admin.from(table).select(cols).or(filter).limit(limit);
      ...
      case "update_lead": {
        ...
        const { data: lead, error: readErr } = await admin
          .from("admin_leads").select("id,name,company,stage,owner_id").eq("id", id).maybeSingle();
        ...
        const note = patch.stage === "won"
          ? " The stage moved, but no client record was created — press Won on the Sales page to do that."
          : "";
