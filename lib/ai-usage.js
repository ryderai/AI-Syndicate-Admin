 ...
 */
/* VENDORS THAT HAVE NO TOKENS AT ALL, EVER — recorded by THIS console.
 *
 * ⭐ WHY THIS EXISTS, AND IT IS THE SAME DEFECT TWICE.
 *
 * api/usage-ingest.js — the path the PLATFORM's spend arrives on — already
 * knows the difference between "this vendor has no tokens by its nature" and
 * "this call failed to report its tokens". It has to: a provider missing from
 * its list gets stamped meta.tokensUnknown, which the AI Cost page renders in
 * its "N calls reported no token counts" banner (the metering-is-broken
 * banner), and its per-call price is discarded so adding a price row later
 * changes nothing on screen. searchapi and zernio hit exactly that on 8 Sep
 * 2026.
 *
 * recordAiUsage — the path THIS console's own spend takes — writes straight to
 * admin_usage_events and never learned the distinction. It stamps
 * tokensUnknown on any row with no usage. So the moment we record a per-credit
 * vendor here (Apollo is the one waiting), it would be reported as a failed
 * measurement rather than a real call we cannot price from tokens.
 *
 * ⭐ AND A PRICE ROW ALONE WILL NOT MAKE THEIR COST APPEAR. `costMicros()`
 * returns null whenever `usage` is null, BEFORE it consults a price, so a
 * per-search fee cannot be expressed in the price book as it stands — it needs
 * the `per_call_micros` column that is an open item in both repos. An earlier
 * version of this comment said "add a price row and it prices itself", which
 * was wrong and would have sent somebody to add a row that changed nothing.
 * Until that column exists these rows read NOT PRICED, which is honest and
 * visible, and their call count is the number that means something.
 *
 * NOT the same list as api/usage-ingest.js's ADMIN_TOKENLESS_PROVIDERS: that
 * one mirrors the PLATFORM's providers and must match it exactly. This one is
 * for vendors this console calls itself. scripts/check-ai-usage.mjs holds both
 * rules apart on purpose. */
export const LOCAL_TOKENLESS_PROVIDERS = new Set(["apollo", "platform-leadgen"]);
  ...
  try {
    ...
    const row = {
      ...
      meta: {
         ...
         *
         * A TOKEN-LESS VENDOR IS NOT THAT. It never had tokens to report, so
         * tokensUnknown would put a working meter in the AI Cost page's
         * metering-is-broken banner. It gets `tokenless` instead, exactly as
         * api/usage-ingest.js does for the platform's side. */
        ...(usage === null || usage === undefined
          ? (LOCAL_TOKENLESS_PROVIDERS.has(String(provider || "").toLowerCase())
              ? { tokenless: true }
              : { tokensUnknown: true })
          : {}),
