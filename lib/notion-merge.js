...
export function mapTask(raw, clients, team) {
  ...
  const client = (clients || []).find((c) => nameKey(c.name) === nameKey(wantClient));
  if (!client) {
  ...
  if (!blankish(raw?.status)) {
    const s = NOTION_STATUS[String(raw.status).trim()] || (STATUSES.includes(raw.status) ? raw.status : null);
    if (s) row.status = s;
    else problems.push(`"${name}": status "${raw.status}" is not one this console has, so it was left alone.`);
  }
  if (!blankish(raw?.priority)) {
    const p = NOTION_PRIORITY[String(raw.priority).trim()] || (PRIORITIES.includes(raw.priority) ? raw.priority : null);
    if (p) row.priority = p;
  ...
  for (const e of emails) {
    const m = (team || []).find((t) => t.active !== false && String(t.email || "").toLowerCase() === e);
    if (m) matched.push({ email: e, user_id: m.user_id }); else unmatched.push(e);
  }
  if (matched.length) row.assigned_to = matched[0].user_id;
  for (const e of unmatched) {
    problems.push(`"${name}": ${e} is assigned in Notion but has no account here, so they are not on this task.`);
  }

  const extras = matched.slice(1).map((m) => m.email).concat(unmatched);
  const descParts = [];
...
export function planTaskImport(list, { clients, team, existing }) {
  ...
  for (const raw of list) {
    ...
    /* RULE 3 and RULE 4 live here. */
    const patch = { id: found.id };
    let changed = false;
    for (const [k, v] of Object.entries(row)) {
      if (k === "client_id" || k === "name") continue;
      if (blankish(v)) continue;                                   // rule 3
      if (k === "status" && found.status === "done" && v !== "done") continue; // rule 4
      if (found[k] === v) continue;
      patch[k] = v; changed = true;
    }
    if (changed) out.update.push({ patch, clientName, name: row.name, was: found });
