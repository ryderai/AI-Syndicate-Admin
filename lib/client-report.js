...
export function assembleReportFacts({
  ...
  /* 24 Sep 2026: money is owners only. A report written for an admin is
   * built with no invoices AND says money was left out — never "no invoices
   * exist", which would be a false statement about the client. */
  moneyHidden = false,
}) {
...
export function missingFrom(facts) {
  ...
  if (facts.money.hidden) gaps.push("Money. Invoices are owners only and were left out of this report.");
  else if (!facts.money.invoices) gaps.push("Money. No invoices exist for this client in the console.");
...
export function buildFactsText(facts, { maxChars = 13000 } = {}) {
  ...
  if (m.hidden) {
    tail.push("- Left out: money is owners only. Say nothing about invoices, payments or amounts owed.");
  } else if (!m.invoices) {
...
export function deterministicReport(facts, { presetId = DEFAULT_PRESET, todayIso } = {}) {
  ...
  b.push(`## Money`);
  if (m.hidden) {
    b.push("- Left out of this report — money is owners only.");
  } else if (!m.invoices) {
