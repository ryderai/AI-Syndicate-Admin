...
export function assembleReportFacts({
  ...
  return {
    ...
    history: previousReports.slice(0, 5).map((r) => ({
      ...
      summaryFirstLine: String(r.summary || "").split("\n").find(Boolean) || null,
    })),
  };
}
 ...
 * true report.
 */
export function buildReportInstruction({ clientName, userInstruction, presetId, todayIso }) {
  const preset = presetById(presetId);
  const typed = String(userInstruction || "").trim().slice(0, MAX_INSTRUCTION_CHARS);

  return `Write an internal report about ${clientName} for the AI Syndicate team, from the FACTS below and NOTHING else. Today is ${todayIso}.

HOW LONG AND HOW DEEP — this is what the person asked for:
<<<
${typed || preset.instruction}
>>>
Aim for about ${preset.words} words in the full version unless the request above clearly asks for more or less.

Output EXACTLY this shape and nothing outside it:

TITLE: one line naming the client and what this report covers.

SUMMARY
- three to five bullets. This is the 30-second version, and it is the part that gets forwarded. A smart 12-year-old must understand every line with no other reading.

REPORT
The full version. Use short paragraphs and "## " headings. Cover, in this order, skipping anything the facts are silent on: where the work stands, what is finished, what is open, what is blocked and what it is waiting on, money, email and follow-ups, websites, and what happens next.
...
export function checkReport(report, factsText, { clientName = "", teamNames = [] } = {}) {
  ...
  if (!report.summary?.trim()) return { ok: false, why: "the 30-second summary came back empty" };
  if (!report.body?.trim()) return { ok: false, why: "the full version came back empty" };
  return { ok: true };
}
...
export function deterministicReport(facts, { presetId = DEFAULT_PRESET, todayIso } = {}) {
  ...
  b.push(`Everything below is counted from this console's own records${facts.takenAt ? ` on ${facts.takenAt.slice(0, 10)}` : ""}. Nothing here is a scan result or a ranking.`);
...
export function reportToMarkdown(report, { clientName, facts, source, instruction }) {
  ...
  parts.push(`## The 30-second version`);
  parts.push(report.summary || "");
  parts.push(`## The full version`);
