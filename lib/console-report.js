...
You are reading the WHOLE console: every client and their open and finished work, the weekly logs, the websites, every lead and the firms behind them, the lists and proposals, every email thread we hold a row for, tickets, follow-ups, the notes the team wrote, the standing rules. Money (invoices, payments, costs) is NOT in here — it is owners only and lives on the Finance page; never say there are no invoices or that nothing is owed. It is all below. Read across it — the answer is usually in how two sections line up, not in either one alone.
...
export function assembleConsoleFacts(snap, { nowMs = Date.now() } = {}) {
  ...
  return {
    ...
    cannotAnswer: [
      ...
      "Money of any kind — invoices, payments, Stripe, costs. It is owners only and is kept out of these shared reports; it is on the Finance page.",
