...
import {
  parseReport, checkReport, MAX_INSTRUCTION_CHARS as CLIENT_MAX,
...
const SHAPE = `Output EXACTLY this shape and nothing outside it:

TITLE: one line naming what this covers. Not "Overview" — say the actual finding.

SUMMARY
- three to five bullets. Each one is a JUDGEMENT, not a count: what is true and why it matters. "Lakeside has had nothing planned for eleven days while we invoiced them on the 1st" is a summary line. "3 clients, 10 open tasks" is not — that is the input, and the person can already see it on the page. This is the part that gets forwarded.

REPORT
The full version, in short paragraphs under "## " headings. This is where the reading happens, so:
- Connect things. A client with late work AND an unpaid invoice AND a quiet mailbox is one story, not three rows. Say so.
- Rank by consequence, not by table. The thing that costs us money or a client comes first, whatever section of the records it lives in.
- Explain every number you use in the same sentence you use it. "Nine days quiet" means nothing on its own; "nine days quiet, and our own rule is five" is a finding.
- Where the records disagree with each other, say which two rows disagree.
- Where something has not moved, say how long and what it is waiting on.

WATCH OUT
- anything in the records that looks wrong, stale, or contradicts itself. If nothing does, write "- Nothing in the records looks wrong." and stop.

NEVER open a section by restating a total. Do not begin with "There are", "We have", "Currently there are", or a bare count. Lead with the finding and put the number inside the sentence that proves it.`;
...
/** Same four sections as a client report, so one parser serves both. */
...
export function checkConsoleReport(report, factsText, { teamNames = [] } = {}) {
  ...
  if (!String(report.summary || "").trim()) return { ok: false, why: "the summary came back empty" };
  if (!String(report.body || "").trim()) return { ok: false, why: "the report came back empty" };
...
export function consoleReportToMarkdown(report, { facts, source, instruction } = {}) {
  ...
  out.push("## The 30-second version", "", report.summary || "", "", "## The full version", "", report.body || "");
...
const CLOSED_LEAD_STAGES = ["won", "lost", "skip_90", "bad_contact"];
