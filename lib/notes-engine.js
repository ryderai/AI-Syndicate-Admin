...
export const THRESHOLDS = {
  ...
  leadStale: {
    ...
    /* Both halves of the 2 Sep 2026 Meeting split, and the pre-0030 value. The
       comment above promises this copy cannot disagree with the Work page's;
       it disagreed for several hours because only that one was renamed. */
...
export function computeNotes(snap, now = Date.now()) {
  ...
  const unclaimed = (snap.leads || []).filter(
    (l) => !l.owner_id && isOpenStage(l.stage));
