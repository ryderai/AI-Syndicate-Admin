...
export const CLIENT_DELETE_CASCADES = [
  /* admin_task_updates (0029) has no client_id of its own — it hangs off the
   * task — so the derived-from-the-migrations check below never sees it and
   * would never make this screen mention it. Named here by hand, because the
   * progress history is the thing somebody would most regret losing quietly. */
  ["admin_tasks", "every task for this client, and every progress update written on it"],
...
export const CLIENT_DELETE_KEEPS = [
  ...
  /* 0032. The link between a platform workspace and this client is cleared,
   * and the workspace row itself stays — deliberately. Deleting a client must
   * not delete the evidence that a workspace spent money, and the workspace
   * reappears on the unmapped-spend list so the spend does not go quiet.
   *
   * This line was not written by hand. The check in tests/client-delete
   * reads the migrations, found admin_platform_workspaces.client_id the
   * moment 0032 was added, and failed — which is exactly what it is for. The
   * screen would otherwise have carried on listing sixteen tables while
   * touching seventeen. */
  ["admin_platform_workspaces", "their platform workspace stays on the AI cost list, unlinked"],
];
