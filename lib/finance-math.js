...
/* ------------------------------------------------------------------ */
/* MEASURED COSTS MERGED INTO TYPED ONES                               */
/* ------------------------------------------------------------------ */

/** Money out per month: what somebody typed, plus the measured costs for any
 * month they did NOT type.
 *
 * TWO SOURCES, ONE NUMBER, AND THE RULE IS THE SAME FOR BOTH.
 *
 * The console knows two costs for real — Stripe's card fees, and (since
 * 7 Sep 2026) what the platform actually spent on AI tokens. It also lets a
 * person type costs by hand. For any given month a cost is either typed or
 * measured, NEVER both: counting a cost twice overstates spend, understates
 * profit, and is harder to spot than a missing one because the total still
 * looks plausible.
 *
 * The typed figure wins where it exists. A person who typed a number has
 * asserted something; the measured feed is not entitled to argue with it by
 * silently adding itself on top. What the screen does instead is SAY that the
 * two disagree, and let them delete the typed one.
 *
 * A NULL MEASURED COST ADDS NOTHING. `null` means "we could not price this
 * month" — not "it was free". Adding 0 would be the same lie the rest of this
 * codebase spends so much effort avoiding, so it is skipped and reported
 * separately.
 *
 * @param {string[]} months          e.g. ["2026-09", ...]
 * @param {object}   typedByMonth    cents per month, from typed expenses
 * @param {object}   measured        { key: { byMonth, typedMonths } } per source
 * @returns {{ costByMonth, addedByMonth, skippedNull, displacedByTyped }}
 */
export function mergeMeasuredCosts(months = [], typedByMonth = {}, measured = {}) {
  const costByMonth = {};
  const addedByMonth = {};
  const skippedNull = {};
  const displacedByTyped = {};

  for (const key of Object.keys(measured)) {
    addedByMonth[key] = {};
    skippedNull[key] = [];
    displacedByTyped[key] = [];
  }

  for (const m of months) {
    let total = Math.round(Number(typedByMonth[m]) || 0);
    for (const [key, src] of Object.entries(measured)) {
      const typedHere = src?.typedMonths instanceof Set
        ? src.typedMonths.has(m)
        : Array.isArray(src?.typedMonths) && src.typedMonths.includes(m);
      const value = src?.byMonth ? src.byMonth[m] : undefined;

      if (typedHere) {
        addedByMonth[key][m] = 0;
        /* Only worth reporting when there WAS something to displace. */
        if (value != null) displacedByTyped[key].push(m);
        continue;
      }
      if (value == null) {
        addedByMonth[key][m] = 0;
        skippedNull[key].push(m);
        continue;
      }
      const cents = Math.round(Number(value) || 0);
      addedByMonth[key][m] = cents;
      total += cents;
    }
    costByMonth[m] = total;
  }

  return { costByMonth, addedByMonth, skippedNull, displacedByTyped };
}
