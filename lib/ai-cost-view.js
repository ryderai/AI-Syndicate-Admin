...
function blank() {
  return { calls: 0, pricedCalls: 0, costMicros: 0, inputTokens: 0, outputTokens: 0, failed: 0 };
}
function add(t, r) {
  t.calls += r.calls || 0;
...
export function aiCostView(data, range) {
  ...
  for (const r of rows) {
    ...
    if (b) {
      if (ak === NO_ACCOUNT || ak === CONSOLE) b.none += r.cost_micros || 0;
      else b.tagged += r.cost_micros || 0;
      b.calls += r.calls || 0;
    }
  ...
  const accountRows = [...accounts.values()].map((a) => ({
    ...
    unpricedCalls: a.calls - a.pricedCalls,
    jobs: [...a.jobs.values()].sort((x, y) => y.costMicros - x.costMicros || y.calls - x.calls),
    services: [...a.services.values()].sort((x, y) => y.costMicros - x.costMicros || y.calls - x.calls),
  })).sort((x, y) => y.costMicros - x.costMicros || y.credits - x.credits || y.calls - x.calls);

  const accountCost = accountRows.filter((a) => a.info.kind === "workspace" || a.info.kind === "client")
    .reduce((s, a) => s + a.costMicros, 0);

  return {
    range, bucket,
    total: { ...total, unpricedCalls: total.calls - total.pricedCalls },
    accountShare: total.costMicros > 0 ? accountCost / total.costMicros : null,
    accountCostMicros: accountCost,
    accounts: accountRows,
    jobs: [...jobs.values()].map((j) => ({ ...j, accountCount: j.accounts.size })).sort((x, y) => y.costMicros - x.costMicros || y.calls - x.calls),
    services: [...services.values()].map((s) => ({ ...s, unpricedCalls: s.calls - s.pricedCalls })).sort((x, y) => y.costMicros - x.costMicros || y.calls - x.calls),
