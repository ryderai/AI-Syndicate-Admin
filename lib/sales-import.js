...
/* The stage gate, so a spreadsheet cannot land a lead in a stage the console
 * would refuse to let a person set. See landableStage below. */
import { STAGE_REQUIRES } from "./stage-move.js";
 ...
 */
/**
 * A STAGE AN IMPORT CAN ACTUALLY LEAVE A LEAD IN — 2 Sep 2026.
 *
 * Three stages require something a spreadsheet export does not contain: Follow
 * up and both Meeting stages need a date, Proposal needs a proposal record with
 * an amount. `stageFromSheet` was happily returning all four, so an import
 * created exactly the state the gate exists to prevent — and one nobody could
 * fix from the screen, because the console will not let a PERSON set a stage
 * without the thing it needs.
 *
 * Found by a checker, on the same day the gate was being made stricter
 * everywhere else. Reading the sheet's words was audited; writing them was not.
 *
 * THE FACT IS NOT THROWN AWAY, IT MOVES INTO THE NOTE. The lead lands at the
 * nearest stage the sheet's own words support, and the timeline says what the
 * export claimed and what is missing — so a rep can set it properly in one
 * click instead of finding a lead in a stage that lies.
 */
export function landableStage({ stage, note }) {
  const need = STAGE_REQUIRES[stage];
  if (!need) return { stage, note };
  /* Where each gated stage falls back to, and why. Every one of these is a
   * stage the sheet's wording already justifies on its own. */
  const DOWN = {
    follow_up: "contacted",          // they said "following up", so contact happened
    meeting_booked: "in_conversation",   // a meeting is arranged by talking
    meeting_complete: "in_conversation",
    proposal: "in_conversation",     // a proposal goes to somebody you are talking to
  };
  const down = DOWN[stage] || "contacted";
  const missing = need.kind === "date" ? "no date came across in the export" : "no proposal amount came across in the export";
  return { stage: down, note: `${note || `at ${stage} in the sheet`} — ${missing}, so it is sitting at ${down} until somebody sets it` };
}
...
export function stageFromSheet(contacted, status) {
  ...
  /* `not_a_fit`, for the same reason as bad contact info below: 0027 merged
   * skip_90 into it and narrowed the constraint, so this has been producing a
   * value Postgres refuses since 0027 ran. The NOTE keeps the reason — which is
   * the whole point of the merge, a reason rather than a stage. */
  if (/skip|90\+/.test(st)) return { stage: "not_a_fit", note: "marked Skip – 90+ in the sheet (their site scores 90 or above)" };
  /* `not_a_fit`, NOT `bad_contact`. Migration 0027 merged the two — a dead
   * email is a REASON for not-a-fit, not an outcome of its own — and narrowed
   * the database constraint, so this line has been producing a value Postgres
   * refuses ever since 0027 ran. Nobody noticed because the test that checks the
   * importer against the constraint was reading migration 0009. Found 2 Sep 2026
   * when that test was repointed at the CURRENT constraint. */
  if (/bad\s*(contact|info|number|email)/.test(st)) return { stage: "not_a_fit", note: "marked bad contact info in the sheet" };
  ...
  /* A sheet that says "meeting" is saying one was ARRANGED, so it lands in
   * meeting_booked. Nothing in an export tells us it already happened, and
   * guessing complete would put an un-dated meeting into the counted stage. */
  if (/meeting|booked|demo/.test(st)) return { stage: "meeting_booked", note: "meeting booked per the sheet" };
...
export function buildImportPlan(rows, opts = {}) {
  ...
  const leads = withCompany.map((raw) => {
    /* THROUGH landableStage, always. A stage the gate would refuse from a
       person must not arrive from a spreadsheet either. */
    const { stage, note } = landableStage(stageFromSheet(raw.contacted, raw.status));
...
const STAGE_ORDER = {
  ...
  in_conversation: 4,
  /* `meeting` keeps rank 5 for rows imported before the 0030 split; the two
   * halves sit either side of it, so an import can move a lead from booked to
   * complete but never backwards. */
  meeting: 5, meeting_booked: 5, meeting_complete: 5.5, proposal: 6,
  won: 9, lost: 9, not_a_fit: 9, skip_90: 9, bad_contact: 9,
};
/* The two 0027 merged stay in both maps: no import can produce them any more,
 * but a row already holding one must still rank and still read as closed. */
const CLOSED = new Set(["won", "lost", "not_a_fit", "skip_90", "bad_contact"]);
