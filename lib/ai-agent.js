...
export async function converse({ system, messages, tools = [], onToolCall, maxTokens = MAX_TOKENS }) {
  ...
  while (rounds < MAX_ROUNDS) {
    ...
    for (const call of toolUses) {
      ...
      /* `preview` rides along so the receipt can say NOTHING SAVED YET rather
       * than ✓ DID THIS. A tool that deliberately writes nothing and returns
       * ok:true — log_meeting before it is confirmed — is otherwise
       * indistinguishable on screen from one that wrote. */
      actions.push({ tool: call.name, input: call.input || {}, ok: outcome.ok, text: outcome.text, preview: outcome.preview === true });
      results.push({
        type: "tool_result",
        tool_use_id: call.id,
