 ...
 */
/* AI DOLLARS ARE OFF FOR NOW — Ryder, 24 Sep 2026: "no need for ai cost yet
 * in dollars". The AI Cost page counts tokens only, so Finance does not add a
 * metered AI dollar figure to money out either; it shows the tokens beside
 * the costs instead. Pass `aiDollars: true` to put them back — nothing else
 * has to change. A real AI bill can still be typed into the cost list under
 * "AI & APIs", and then it IS money out, like any other bill. */
export const AI_DOLLARS_DEFAULT = false;

export function financeView({ summary, ai, expenses = [], range, bucket: forced = null, today = null, aiDollars = AI_DOLLARS_DEFAULT }) {
  ...
  let aiCalls = 0;
  const aiTokens = { platform: 0, agency: 0, shared: 0 };
  ...
  for (const row of ai?.rows || []) {
    ...
    aiTokens[dept] += (row.input_tokens || 0) + (row.cache_write_tokens || 0) + (row.output_tokens || 0);
    aiCalls += row.calls || 0;
  ...
  return {
    ...
    ai: {
      calls: aiCalls, unpricedCalls: aiUnpricedCalls, via: ai?.via || null, dollars: Boolean(aiDollars),
      tokens: aiTokens, tokensTotal: aiTokens.platform + aiTokens.agency + aiTokens.shared,
    },
